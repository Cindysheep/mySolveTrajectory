...
float completeAirResistanceModel(float s, float v, float angle)
{
    ...
    if (t < C){//rising
        z = log(cos(sqrt(st.k * GRAVITY) * (C - t)) / cos(sqrt(st.k * GRAVITY) * (C))) / st.k;
    }

    else{//falling
        z = log(1 + st.k * (v * sin(angle)) * (v * sin(angle)) / GRAVITY) / (2 * st.k) - GRAVITY * t * t / 2;
    }
    printf("Complete model %f %f\n", t, z);
    return z;
...
float pitchTrajectoryCompensation(float s, float z, float v)
{
    ...
    for (i = 0; i < 20; i++)
    {
        ...
        //z_actual = completeAirResistanceModel(s,v,angle_pitch);
        dz = 0.3*(z - z_actual);
        z_temp = z_temp + dz;
